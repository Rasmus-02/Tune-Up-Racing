...
var id = [Car_ID,7,Part_Number,rarity] #Engine ID, Part type, Part number, rank
...
func _process(_delta):
	if (current_color == null or color != current_color or durability != current_durability or (car != null and car.is_in_group("Car") and current_durability != car.r_bumper_durability)):
		if car != null and car.is_in_group("Car"):
			durability = car.r_bumper_durability
