...
func _process(delta):
	if (current_durability == null or durability != current_durability or (car != null and car.is_in_group("Car") and current_durability != car.subframe_durability)):
		if car != null and car.is_in_group("Car"):
			durability = car.subframe_durability
			apply_durability(car.subframe_durability)
