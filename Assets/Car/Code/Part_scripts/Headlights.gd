...
var id = [Car_ID,4,Part_Number,rarity] #Engine ID, Part type, Part number, rank
...
@export var engine_cool_mod = 0.0
...
func _process(_delta):
	if (current_color == null or color != current_color or durability != current_durability or (car != null and car.is_in_group("Car") and current_durability != car.headlights_durability)):
		if car != null and car.is_in_group("Car"):
			durability = car.headlights_durability
			paint_part(car.headlights_color, car.headlights_durability)
