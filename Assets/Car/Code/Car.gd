...
func wheel_controller():
	if selected_wheels != 0:
		...
		max_tire_limit = (tires.grip * 0.5 * (wheels_on_road * 0.25) + (tires.grip * 0.5)) * rubberbanding * $Placing.grip_bonus
		...
		if (drive_train[1]-1)/1 > (drive_train[0]-1)/0.4:
			...
			if smoke_amount > 0.6:
				...
				tiremark_array[2].draw_tiremark(false, RL.global_position)
				tiremark_array[3].draw_tiremark(false, RR.global_position)
			tiresmoke_instanse.get_child(0).process_material.color = Color(1,1,1,smoke_amount * (drive_train[1]-1)/1)
			tiresmoke_instanse.get_child(0).process_material.scale_max = 3 * (drive_train[1]-1)/1
		#Smoke for rear wheels
			...
			tiresmoke_instanse.get_child(0).process_material.color = Color(1,1,1,smoke_amount * (drive_train[0]-1)/0.6)
			tiresmoke_instanse.get_child(0).process_material.scale_max = 4 * (drive_train[0]-1)/0.4
#endregion
...
func get_input():
	...
	tire_limit -= tirelimit_scale * slide_ammount * 10
	...
	if forward > 0 and clutch <= 0:
		...
		tire_limit -= (tirelimit_scale * (acceleration.length()/30/drivetrain_grip_mod)) * (tire_limit / max_tire_limit)
...
func traction_controller():
	...
	traction_fast = (traction_fast_max * (tire_limit/max_tire_limit))* (1-slide_ammount)
	traction_fast = clamp(traction_fast, 0.007, traction_fast_max)
	#RWD
